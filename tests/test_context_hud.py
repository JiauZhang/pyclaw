...
from pyclaw.tui import (METER_TRACK_LIGHTNESS, context_meter, git_label,
                        usage_hud, usage_meter)
...
def test_context_window_defaults_to_two_hundred_thousand(tmp_path):
    real = config.__config_file__
    config.__config_file__ = tmp_path / 'config.json'
    try:
        config.reload()
        assert config.load()['contextWindow'] == 200_000
    finally:
...
def test_meter_cells_and_percent_track_the_ratio():
    assert _plain(context_meter(TRIPLE, 34_000, 100_000)) == '│███░░░░░░░│ 34%'
    assert _plain(context_meter(TRIPLE, 50_000, 100_000)) == '│█████░░░░░│ 50%'
    assert _plain(context_meter(TRIPLE, 0, 100_000)) == '│░░░░░░░░░░│ 0%'


def test_meter_clamps_to_the_window_it_is_given():
    assert _plain(context_meter(TRIPLE, 250_000, 100_000)) == '│██████████│ 100%'


def test_meter_shrinks_its_cells_on_a_narrow_terminal():
    assert _plain(context_meter(TRIPLE, 40_000, 100_000, cells=5)) == '│██░░░│ 40%'
...
def test_usage_meter_shows_the_empty_track_before_the_first_response():
    ...
    markup = usage_meter(TRIPLE, _Usage())
    assert _plain(markup) == '│░░░░░░░░░░│'
    assert _track_hex(markup) == banner.dimmed(TRIPLE, METER_TRACK_LIGHTNESS)
    assert usage_meter(TRIPLE, _Usage(prompt=10, completion=5, total=15),
                       cells=0) == ''
...
def test_usage_meter_is_one_bar_of_three_kinds_of_token():
    ...
    markup = usage_meter(TRIPLE, _Usage(prompt=1901, completion=500,
...
def test_usage_meter_gives_a_cell_to_every_kind_that_exists():
    ...
    markup = usage_meter(TRIPLE, _Usage(prompt=200_000, completion=20,
...
def test_usage_hud_lists_the_numbers_the_bar_is_made_of():
    ...
    assert usage_hud(_Usage(prompt=61_200, completion=3_400, total=64_600,
                            cached=48_000)) == (
        'in: 61.2k  out: 3.4k  cache: 78%  total: 64.6k')


def test_usage_hud_reads_zero_before_the_first_response():
    assert usage_hud(_Usage()) == 'in: 0  out: 0  cache: 0%  total: 0'


def test_usage_hud_reports_a_zero_hit_rate():
    assert usage_hud(_Usage(prompt=1_000, completion=20, total=1_020)) == \
